...
import { ArrowLeft, Shield, Send, FileText } from 'lucide-react';
...
export default function RequestSenderID() {
  const [formData, setFormData] = useState({
    ...
    organization: '',
    sample_message: '',
    student_file: null as File | null
  });
  ...
  // Prefilled EasyTextAPI values for students
  const studentDefaults = {
    sender_id: 'EasyTextAPI',
    organization: 'SEWMR Technologies',
      ...
      if (isStudent) {
        if (!formData.student_file) {
          toast({
            variant: 'destructive',
            title: 'Missing File',
            description: 'Please upload your student ID.'
          });
          setLoading(false);
          return;
        }
        ...
        form.append('is_student_request', 'true');
        form.append('file', formData.student_file);
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <Card>
            ...
            <CardContent>
              <form onSubmit={handleSubmit} className="space-y-6">
                  ...
                  <input
                    ...
                    onChange={(e) => {
                      ...
                      if (e.target.checked) {
                        setFormData(prev => ({
                          ...prev,
                          sender_id: studentDefaults.sender_id,
                          organization: studentDefaults.organization,
                          sample_message: studentDefaults.sample_message
                        }));
                      }
                    }}
                  />
                  <Label htmlFor="student_toggle">I am a student</Label>
                </div>
                ...
                {isStudent && (
                  <div className="space-y-2">
                    <Label htmlFor="student_file">Upload Student ID (PDF only) <span className="text-destructive">*</span></Label>
                    <Input
                      id="student_file"
                      type="file"
                      accept="application/pdf"
                      onChange={(e) => setFormData(prev => ({ ...prev, student_file: e.target.files?.[0] || null }))}
                    />
                    <p className="text-sm text-muted-foreground">
                      EasyTextAPI is a default sender ID for students. It allows you to send test/project messages without needing a business-approved sender ID.
                    </p>
                  </div>
                )}
        ...
        <div className="space-y-6">
          ...
          {isStudent && (
            <Card>
              <CardHeader>
                <CardTitle className="text-lg">About EasyTextAPI for Students</CardTitle>
              </CardHeader>
              <CardContent className="space-y-4">
                <p className="text-sm text-muted-foreground">
                  EasyTextAPI is the default sender ID provided to students. You can send test or project messages without needing a business-approved sender ID. It's pre-configured for student use and allows immediate testing or integration of SMS functionality.
                </p>
                <ul className="text-sm text-muted-foreground space-y-1">
                  <li>• Pre-configured for student use</li>
                  <li>• No setup required</li>
                  <li>• Messages come from EasyTextAPI as the sender ID</li>
                  <li>• Temporary for academic/testing purposes</li>
                </ul>
              </CardContent>
            </Card>
          )}
